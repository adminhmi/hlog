...
func TestSentryStacktrace(t *testing.T) {
	WithTestDSN(t, func(dsn string, pch <-chan *resultPacket) {
		...
		expectedSuffix := "hlog_sentry/sentry_stacktrace_test.go"
		...
		hook.StacktraceConfiguration.InAppPrefixes = []string{"github.com/adminhmi/hlog"}
		...
		expectedFilename := "github.com/adminhmi/hlog/entry.go"
