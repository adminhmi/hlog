...
func getTestLogger() *hlog.Logger {
	l := hlog.New()
...
func TestSpecialFields(t *testing.T) {
	WithTestDSN(t, func(dsn string, pch <-chan *resultPacket) {
		...
		hook, err := NewSentryHook(dsn, []hlog.Level{
			hlog.ErrorLevel,
		})
...
func TestSentryWithClient(t *testing.T) {
	WithTestDSN(t, func(dsn string, pch <-chan *resultPacket) {
		...
		hook, err := NewWithClientSentryHook(client, []hlog.Level{
			hlog.ErrorLevel,
		})
		if err != nil {
			t.Fatal(err.Error())
